...
impl Input<'_> {
    fn read(&mut self) -> String {
        ...
        let command = self.logger.message.replace(ParserMessage {
            autocomplete: String::from(""),
        });
        let autocomplete_string = &command.autocomplete;

        if !autocomplete_string.is_empty() && !expression.ends_with(autocomplete_string) {
            return expression + autocomplete_string;
        }
    ...
    fn parse(&mut self) -> String {
        ...
        while &value != "end" {
            ...
            if v.starts_with("block_start:") {
                ...
                self.logger.message.replace(ParserMessage {
                    autocomplete: String::from("}"),
                });
...
fn main() {
    ...
    let message = RefCell::new(ParserMessage {
        autocomplete: String::new(),
    });
    let logger = &Logger { message };
    let mut p = Parser {
        logger,
        lexer: Lexer {
